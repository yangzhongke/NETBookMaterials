    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            //var orderedItems1 = list.OrderBy(e=>e.Age).OrderByDescending(e=>e.Salary);
            /*
var orderedItems1 = list.OrderBy(e => e.Age).ThenByDescending(e => e.Salary);
foreach (var item in orderedItems1)
{
    Console.WriteLine(item);
}*/
            ...
            /*
            var items = list.OrderBy(e => Guid.NewGuid());
...
foreach (var item in items)
{
    Console.WriteLine($"性别{item.Gender},人数{item.Count},平均工资{item.AvgSalary:F},最小年龄{item.MinAge}");
            ...
            var items1 = list.Where(e => e.Salary > 3000).OrderBy(e => e.Age).Select(e=>new {e.Name,e.Age,Gender=e.Gender?"男":"女"});
var items2 = from e in list
            ...
            //计算一个逗号分隔字符串数字的和，计算一个字符串中。很多算法问题都可以用LINQ轻松解决，虽然说效率不一定是最好的。
        }
    }
}
