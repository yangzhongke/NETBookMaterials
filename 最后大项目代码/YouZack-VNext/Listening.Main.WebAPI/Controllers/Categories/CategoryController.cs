...
public class CategoryController : ControllerBase
{
    ...
    public CategoryController(ListeningDbContext dbContext, IMemoryCacheHelper cacheHelper)
    {
    ...
    public async Task<ActionResult<CategoryVM[]>> FindAll()
    {
        ...
        {
            return dbContext.Query<Category>().OrderBy(c => c.SequenceNumber).ToArrayAsync();
        }
    ...
    public async Task<ActionResult<CategoryVM?>> FindById([RequiredGuid] Guid id)
    {
        var cat = await cacheHelper.GetOrCreateAsync($"CategoryController.FindById.{id}",
            async (e) => CategoryVM.Create(await dbContext.FindAsync<Category>(id)));
